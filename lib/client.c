...
static inline int get_response(u64 id, int sock, struct kv_response *resp)
{
    ...
    event.data.fd = sock;
    if (epoll_ctl(state.epfd, EPOLL_CTL_ADD, sock, &event)) {
        err_cache = errno;
...
remove:
    memset(&event, 0, sizeof(struct epoll_event));
    event.events = EPOLLIN;
    epoll_ctl(state.epfd, EPOLL_CTL_DEL, sock, &event);
...
static inline u64 fill_request_header(struct kv_request *req, enum kv_request_type type, u16 length)
{
...
int kernkv_get(u64 key, struct value *buf)
{
    u64 req_id;
    ...
    out = get_response(req_id, resp_sock, &resp);
    if (out != KV_SUCCESS && out != KV_VALUE) {
        err_cache = errno;
        goto out_close;
    }
